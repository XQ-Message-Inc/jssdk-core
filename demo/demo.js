...
function buildEncryptScreen() {
    ...
    $("label[for='label-content']")
        .empty()
        .html("Its time to add emails of the folks who you want to be able to read your message.<br />"
    ...
    $("input[id='encrypt-input']").attr('placeholder', "Your message goes here.")
        .focus()
        .show();
...
function buildIdentifyScreen() {
  ...
  $("input[id='encrypt-input']")
    ...
    $("input[id='recipient-input']").attr('placeholder', "Enter Your Recipient Email")
        .empty()
        .focus()
        .show();
}
...
function buildDecryptScreen() {
    ...
    $("#sender-access-request-button, #validate-access-request-button, #recipient-list-button, #recipient-access-request-button, #encrypt-button, #decrypt-button").hide();
    $("#user-input, #pin-input, #recipient-list-input, #recipient-input, #encrypt-input, #decrypt-input").hide();
    ...
    $("#message-content")
        .empty()
        .html("<span><b class='client-name'>" + getProperty("decryptedText") + "<br /></span>");
...
window.addEventListener("load",
    function initScreen() {
        ...
        $.each(selections, function(val, text) {
            selectBox.append(
                $('<option style="font-family: ProximaNova, sans-serif; font-size: 24px;font-weight: 500;font-variant: normal;color: dodgerblue;"></option>').val(val).html(text)
            );
        });
