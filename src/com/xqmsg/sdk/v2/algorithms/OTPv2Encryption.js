...
export default class OTPv2Encryption extends EncryptionAlgorithm {
        ...
        try {
            ...
            let stream = new ReadableStream({
                async start(controller) {
                    controller.enqueue(new Uint8Array(new Uint32Array([token.length]).buffer));
            ...
            return new Response(stream, {headers: {'Content-Type': 'octet/stream'},})
                .blob()
                .then(function (bob) {
                        return new Promise(function (resolve, reject) {
                            resolve(new ServerResponse(
    ...
    decryptFile = function ( file, retrieveKeyFunction) {
        ...
        try {
            ...
            return new Promise(function (resolve, reject) {
                let parsedFile;
                return self.parseFileForDecrypt(file)
                    .then(function (pf) {
                        parsedFile = pf;
               ...
               // class because Safari does not support Blob.arrayBuffer
                 resolve( file
                           ...
                           .then(function(buffer){
                               ...
                               return  {locator:locator, nameEncrypted:nameEncrypted, contentEncrypted: new Uint8Array(buffer.slice(pos))};
